function CStack(oInfo, oColor, aEdge, oParentContainer) {
    ...
    this._init = function (oInfo, oColor, aEdge) {
        ...
        this.setEdges(aEdge, {x: _oContainer.x, y: _oContainer.y});
...
//        _bCached = true;
    ...
    this.createDepthEdge = function () {
        ...
        _aEdgesDepth.push({x: _aEdges[1].getPointA().getX(), y: _aEdges[1].getPointA().getY()});//RIGHTDOWN
        _aEdgesDepth.push({x: _aEdges[1].getPointB().getX(), y: _aEdges[1].getPointB().getY()});//CENTER
        _aEdgesDepth.push({x: _aEdges[2].getPointB().getX(), y: _aEdges[2].getPointB().getY()});//LEFTDOWN
        _aEdgesDepth.push({x: _aEdges[2].getPointB().getX(), y: _aEdges[2].getPointB().getY() + STACK_DEPTH_SIZE});//LEFTDOWN ISOMETRIC
        _aEdgesDepth.push({x: _aEdges[1].getPointB().getX(), y: _aEdges[1].getPointB().getY() + STACK_DEPTH_SIZE});//CENTER ISOMETRIC
        _aEdgesDepth.push({x: _aEdges[1].getPointA().getX(), y: _aEdges[1].getPointA().getY() + STACK_DEPTH_SIZE});//RIGHTDOWN ISOMETRIC
    };
    ...
    this.perfectPosition = function () {
        ...
        var oParent = this;
        new colorLerpShape(_oLines, {r: oStartColorRGB.r, g: oStartColorRGB.g, b: oStartColorRGB.b, lerp: 0.0},
                {r: oEndColorRGB.r, g: oEndColorRGB.g, b: oEndColorRGB.b, lerp: 1.0},0, MS_TIME_PERFECT_HIT, this.createGraph, createjs.Ease.cubicOut, _aEdges, "plane", function () {
                    
            new colorLerpShape(_oLines, {r: oEndColorRGB.r, g: oEndColorRGB.g, b: oEndColorRGB.b, lerp: 0.0},
                    {r: oStartColorRGB.r, g: oStartColorRGB.g, b: oStartColorRGB.b, lerp: 1.0},0, MS_TIME_PERFECT_HIT, oParent.createGraph, createjs.Ease.cubicOut, _aEdges, "plane", null);
        });

        var oStartColorRGBDepth = hexToRgb(_oColor.depth);
        var oEndColorRGBDepth = hexToRgb(colorLuminance(_oColor.depth, 0.5));
        new colorLerpShape(_oDepth, {r: oStartColorRGBDepth.r, g: oStartColorRGBDepth.g, b: oStartColorRGBDepth.b, lerp: 0.0},
                {r: oEndColorRGBDepth.r, g: oEndColorRGBDepth.g, b: oEndColorRGBDepth.b, lerp: 1.0},0, MS_TIME_PERFECT_HIT, this.updateDepthEffect, createjs.Ease.cubicOut, _aEdgesDepth, "depth", function () {
            new colorLerpShape(_oDepth, {r: oEndColorRGBDepth.r, g: oEndColorRGBDepth.g, b: oEndColorRGBDepth.b, lerp: 0.0},
                    {r: oStartColorRGBDepth.r, g: oStartColorRGBDepth.g, b: oStartColorRGBDepth.b, lerp: 1.0},0, MS_TIME_PERFECT_HIT, oParent.updateDepthEffect, createjs.Ease.cubicOut, _aEdgesDepth, "depth", null);
        });
    };
    ...
    this.fallEffect = function () {
        createjs.Tween.get(_oContainer).to({y: CANVAS_HEIGHT, alpha: 0}, 1500, createjs.Ease.cubicIn).call(function () {
